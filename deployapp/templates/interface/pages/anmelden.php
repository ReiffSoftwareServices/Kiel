<div class="container mt-4">
	<div class="card">
	  <div class="card-body">
		<h5 class="card-title">Gerüst-Anmeldung</h5>
		<h6 class="card-subtitle mb-2 text-muted">Anmeldung eines neuen Gerüstes.</h6>
		<p class="card-text">
			<form>
			  <div class="form-group">
				<label for="exampleInputEmail1">Anforderer</label>
				<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Christoph Lehnertz">
				<small id="emailHelp" class="form-text text-muted">Name der Person, welche das Gerüst anfordert.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Gerüstnummer</label>
				<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="3">
				<small id="emailHelp" class="form-text text-muted">Die Gerüstnummer wird automatisch ermittelt.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Firma</label>
				<select class="form-control" id="exampleFormControlSelect1" placeholder="Firma Name"><option>Firma auswählen...</option><option>Firma 1</option><option>Firma 2</option><option>Firma 3</option><option>Firma 4</option></select>
				<small id="emailHelp" class="form-text text-muted">Firma auswählen, welche das Gerüst anfordert.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Ansprechpartner</label>
				<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Beispieltext">
				<small id="emailHelp" class="form-text text-muted">Ansprechpartner für die Bestellung in der Firma.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Anlage/Equipment</label>
				<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Beispieltext">
				<small id="emailHelp" class="form-text text-muted">Anlagenarten wie xy.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Ebene</label>
				<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Beispieltext">
				<small id="emailHelp" class="form-text text-muted">Eben in m oder Ebenennummer eingeben.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Örtlichkeit</label>
				<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Beispieltext">
				<small id="emailHelp" class="form-text text-muted">Koordinaten wie beispielsweise A12.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Grund</label>
				<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Beispieltext">
				<small id="emailHelp" class="form-text text-muted">Kurze Begründung warum das Gerüst angefordert wird.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Größe</label>
				<div class="input-group">
					<input type="text" class="form-control" placeholder="L">
					<input type="text" class="form-control" placeholder="B">
					<input type="text" class="form-control" placeholder="H">
				</div>
				<small id="emailHelp" class="form-text text-muted">Maße des Gerüstes.</small>
			  </div>
			  <div class="form-group">
				<label for="exampleInputEmail1">Zeitraum</label>
				<div class="input-group">
						<input type="text" class="form-control" placeholder="Nutzungsbeginn">
						<input type="text" class="form-control" placeholder="Nutzungsdauer">
				</div>
				<small id="emailHelp" class="form-text text-muted">Zeitrum in dem das Gerüst genutzt wird.</small>
			  </div>
			  <button type="submit" class="btn btn-secondary">Gerüst anmelden</button>
			</form>	
		</p>
	  </div>
	</div>
</div>