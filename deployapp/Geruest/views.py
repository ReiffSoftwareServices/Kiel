...
from geruestproject.api.serializers import UserSerializer, GroupSerializer, ClientSerializer
...
def index(request):
    ...
    return render(request, 'interface/index.php',
                  {'new_scaffold_number': new_scaffold_number, 'firmen_list': firmen_list, 'heute': heute,
                   'geruestnummer_liste': geruestnummer_liste, 'leistungsverzeichnis': leistungsverzeichnis,
...
def save_anmeldung(request):
    ...
    print(x)
    q = Geruestbuch( \
        Projekt=p, Anforderer=request.POST['Anforderer'], Geruestbezeichner=x, Firma=e,
        Ansprechpartner=request.POST['Ansprechpartner'], AnlageEquipment=request.POST['AnlageEquipment'],
        Ebene=request.POST['Ebene'], Oertlichkeit=request.POST['Oertlichkeit'], Grund=request.POST['Grund'],
        L=request.POST['L'], B=request.POST['B'], H=request.POST['H'], Nutzungsbeginn=request.POST['Nutzungsbeginn'],
        Status="Angemeldet", Positionsart="Hauptgerüst")
    q.save()
